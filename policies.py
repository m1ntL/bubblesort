...
class LinkedSet(object):
    ...
    class _Node(object):
        ...
        def __init__(self, val, up=None, down=None):
            ...
            """
            #luqi add self.freq to record the requency of request or response of current content 
            self.val = val
            self.freq = 0
            self.up = up
            self.down = down
...
@register_cache_policy('RSF')
class RsfCache(Cache):
    @inheritdoc(Cache)
    def __init__(self, maxlen, **kwargs):
        #pdb.set_trace()
        self._cache = LinkedSet()
        self._maxlen = int(maxlen)
      #  print("maxlen=%d" %self._maxlen)
        if self._maxlen <= 0:
            raise ValueError('maxlen must be positive')
        self._rqft = LinkedSet()
        self._rsft1 = LinkedSet()
        self._rsft2 = LinkedSet()
    ...
    def get(self, k, *args, **kwargs):
        ...
        if k in self._rqft:
            ...
            while curr:
                if curr.val == k:
                    curr.freq += 1
                    if curr != self._rqft._top:
                        while curr.up and curr.freq > curr.up.freq:
                            self._rqft.move_up(k)
                    break
                curr = curr.down 
        else :
            if len(self._rqft) >= 2 * self._maxlen:
               #pdb.set_trace()
                self._rqft.pop_bottom()
            self._rqft.append_bottom(k)
            self._rqft._bottom.freq += 1
        # lookup content in cache queue 
        if k in self._cache:
            #update self._rsft 
            if k in self._rsft1: #in cache 
                curr = self._rsft1._bottom  
                while curr:
                    if curr.val == k:
                        curr.freq += 1
                        while curr and curr.up and curr.freq > curr.up.freq:
                            self._rsft1.move_up(k)
                    break 
                    curr = curr.up 
            """
            elif k in self._rsft2: # not in cache 
                curr = self._rsft2._top 
                while curr:
                    if curr.val == k:
                        curr.freq += 1
                        if curr != self._rsft2._top:
                            while curr.up and curr.freq > curr.up.freq:
                                self._rsft2.move_up(k)
            """
            """
            else :
                if len(self._rsft1) >= self._maxlen:
                    freq = self._rsft1._bottom.freq 
                    k = self._rsft1.pop_bottom()
                    self._rsft2.append_top(k)
                    self._rsft2._top.freq = freq
                    curr = self._rsft2._top 
                    while curr.down and curr.freq < curr.down.freq:
                        self._rstf2.move_down(curr.val)
                        curr = curr.down 
            """    
                #self._rsft1.append_bottom(k)
            return True 
        else : return False
    ...
    def put(self, k, *args, **kwargs):
        ...
        if k in self._cache:
            ...
            if len(self._cache) >=  self._maxlen :
                 #cache has no space 
                f2 = self._rsft1._bottom.freq
                if len(self._rsft2) >= self._maxlen:
                    self._rsft2.pop_bottom()
                v2 = self._rsft1.pop_bottom()
                self._rsft2.append_bottom(v2)
                self._rsft2._bottom.freq = f2 
                tmp = self._rsft2._bottom 
                while tmp and tmp.up and tmp.freq > tmp.up.freq :
                    self._rsft2.move_up(tmp.val)
                self._cache.remove(v2) 
            # cache has space, add to rsft1
            self._cache.append_bottom(k)
            #add k into _rsft1 
            if k in self._rsft2: 
                if len(self._rsft1) >= self._maxlen:  # t1 has no space,evict an item to rsft2  
                    f = self._rsft1._bottom.freq 
                    v = self._rsft1._bottom.val 
                    self._rsft2.append_bottom(v)
                    self._rsft2._bottom.freq = f
                    curr = self._rsft2._bottom 
                    while curr and curr.up and curr.freq > curr.up.freq :
                        self._rsft2.move_up(curr.val)
                        curr = curr.up 
                # find k in rsft2 and move it to t1, then adjust t1  
                curr = self._rsft2._top 
                while curr:
                    if curr.val == k:
                        freqk = curr.freq  #in rsft2  
                        self._rsft2.remove(k)     #delete in t2 
                        self._rsft1.append_bottom(k)  #add to t1 
                        self._rsft1._bottom.freq = freqk  
                        tmp = self._rsft1._bottom         # adjust t1 
                        while tmp and tmp.up and tmp.freq > tmp.up.freq:
                            self._rsft1.move_up(tmp.val)
                            tmp = tmp.up   
                        break;
                    curr = curr.down 
            else : # k is not in t2, k.freq = 0 
                if len(self._rsft1) >= self._maxlen: # t1 has no spece 
                    if len(self._rsft2) >= self._maxlen:
                        # t2 has no  space 
                        self._rsft2.pop_bottom()
                    f1 = self._rsft1._bottom.freq
                    v1 = self._rsft1.pop_bottom()
                    self._rsft2.append_bottom(v1)
                    self._rsft2._bottom.freq = f1
                    tmp = self._rsft2._bottom 
                    while tmp and tmp.up and tmp.freq > tmp.up.freq:
                        self._rsft2.move_up(tmp.val)
                        tmp = tmp.up 
